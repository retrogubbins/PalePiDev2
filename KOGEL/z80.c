...
#include "../PALESDL.H"
//extern void tracedump(UWORD pc);
...
//MYSTUFF
/*
unsigned int trapthis=0;
unsigned int trapped=0;
unsigned int trapaddr=0xffff;  //assumes PC never gets here
int input_pending=0;
dword ip_port;
byte ip_byt;
byte ip_byt_orig;
byte ip_shift=0;
unsigned char blank_mono=0;
*/
...
void saveTables()
{
		...
		char lbl[50];
		int i;
		otpf=fopen("THETABLES.txt","wt");
        if(otpf==NULL)
        {
                sprintf(lbl,"Cannot open TABLES File for writing");
				//gui_error(lbl);
                return;
        }
        fprintf(otpf,"{\n");
        fprintf(otpf,"//Hello World - these lines included from the PaleSDL Table saving routine\n");
        fprintf(otpf,"\n//ZSTable\n");
	for (i=0;i<512;++i)
 	{
		fprintf(otpf,"%d,\n",  ZSTable[i]);
...
//Save the tables for use in constant arrays in microcontroller ports of the emulator
// saveTables();
...
UWORD z80_get_reg(z80_register reg)
{
	...
	if(reg==Z80_REG_AF)return((UWORD)R.PC.W.l);
	if(reg==Z80_REG_HL)return((UWORD)R.PC.W.l);
	if(reg==Z80_REG_DE)return((UWORD)R.PC.W.l);
	if(reg==Z80_REG_BC)return((UWORD)R.PC.W.l);
	if(reg==Z80_REG_SP)return((UWORD)R.PC.W.l);
	if(reg==Z80_REG_IX)return((UWORD)R.PC.W.l);
	if(reg==Z80_REG_IY)return((UWORD)R.PC.W.l);
